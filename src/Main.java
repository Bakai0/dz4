...
public class Main {
    ...
    public static String heroesAttackType[] = {"Mechnik", "Mag", "Telepat", "Medic","Golem","Lucky"};
    public static int heroesHealth[] = {250, 200, 220, 250,500,190};
    public static int heroesDamage[] = {25, 20, 20, 0,15,10};
        ...
        for (int i = 0; i < heroesAttackType.length; i++) {
            if (heroesAttackType[i] == "Medic") {
                for (int j = 0; j < heroesAttackType.length; j++) {
                    if (heroesHealth[i] > 0 && heroesHealth[j] < 100 && heroesHealth[j] > 0 && heroesAttackType[j] != heroesAttackType[i]) {
                        heroesHealth[j] = heroesHealth[j] + 60;
